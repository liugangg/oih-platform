...
class QwenBioAgent:
    ...
    async def chat(self, user_message: str) -> str:
        ...
        while iteration < max_iterations:
            ...
            # Parse tool calls from reasoning field (Qwen3 thinking mode)
            import re as _re
            if not message.get("tool_calls") and message.get("reasoning"):
                tc_matches = _re.findall(r"<tool_call>\s*(.*?)\s*</tool_call>", message["reasoning"], _re.DOTALL)
                if tc_matches:
                    parsed = []
                    for i, tc_json in enumerate(tc_matches):
                        try:
                            tc_data = json.loads(tc_json)
                            parsed.append({"id": f"call_{iteration}_{i}", "type": "function",
                                "function": {"name": tc_data["name"], "arguments": json.dumps(tc_data.get("arguments", {}))}})
            ...
            if not message.get("tool_calls") and finish_reason == "stop":
